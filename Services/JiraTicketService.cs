...
namespace MeetingTranscriptProcessor.Services
{
    ...
    public class JiraTicketService : IDisposable
    {
        ...
        public JiraTicketService(AzureOpenAIService aiService, ILogger? logger = null)
        {
            ...
            _defaultProjectKey = Environment.GetEnvironmentVariable("JIRA_PROJECT_KEY") ??
                                Environment.GetEnvironmentVariable("JIRA_DEFAULT_PROJECT") ?? "TASK";
            _logger = logger;
            ...
            MeetingTranscript transcript)
        {
            ...
            try
            {
                Console.WriteLine($"🎫 Processing {transcript.ActionItems.Count} action items for Jira tickets");
                ...
                foreach (var actionItem in transcript.ActionItems)
                {
                    ...
                    if (!string.IsNullOrEmpty(actionItem.ExistingTicketKey))
                    {
                        ...
                        if (ticketResult.Success) result.TicketsUpdated++;
                    }
                    else
                    {
                        // Create new ticket
                        ticketResult = await CreateNewTicketAsync(actionItem, transcript);
                        if (ticketResult.Success) result.TicketsCreated++;
                    }
                ...
                Console.WriteLine($"✅ Processed {result.TicketsCreated} created, {result.TicketsUpdated} updated tickets");
            ...
            MeetingTranscript transcript)
        {
            try
            {
                ...
                var issuePayload = new
                {
                    fields = new
                    {
                        ...
                        issuetype = new { name = MapActionItemTypeToJiraIssueType(formattedTicket.Type) },
                ...
                var json = JsonSerializer.Serialize(issuePayload, new JsonSerializerOptions { WriteIndented = true });
                ...
                _logger?.LogError(new Exception($"Jira API error: {response.StatusCode}"), errorContent);
            ...
            {
                _logger?.LogError(ex, $"Error creating Jira ticket for action item: {actionItem.Title}");
                return await SimulateTicketCreationAsync(actionItem, TicketOperation.Created);
            }
        }
            ...
            TicketOperation operation)
        {
            ...
            var ticketKey = operation == TicketOperation.Updated && !string.IsNullOrEmpty(actionItem.ExistingTicketKey)
        ...
        private static object CreateJiraDescriptionADF(string description, MeetingTranscript transcript)
        {
            ...
            content.Add(new
            {
                ...
                content = new object[]
                {
                    ...
                    {
                        type = "text",
                        text = transcript.MeetingDate.ToString("yyyy-MM-dd")
                    }
                }
            });
            ...
            if (transcript.Participants.Any())
            {
                content.Add(new
                {
                    ...
                    content = new object[]
                    {
                        ...
                        {
                            type = "text",
                            text = string.Join(", ", transcript.Participants)
                        }
                    }
                });
            }
        ...
        private static string CreateJiraDescription(ActionItem actionItem, MeetingTranscript transcript)
        {
            ...
            {
                description.AppendLine();
                description.AppendLine($"*Meeting Participants:* {string.Join(", ", transcript.Participants)}");
            }
        ...
        /// </summary>
        private async Task<FormattedTicket> FormatTicketWithAIAsync(ActionItem actionItem, MeetingTranscript transcript)
        {
            try
            {
                ...
                return new FormattedTicket
                {
                    ...
                    Priority = Enum.TryParse<ActionItemPriority>(
                        formattedData.TryGetProperty("priority", out var priorityElement)
                            ...
                            : "Medium", out var priority) ? priority : ActionItemPriority.Medium,
                    Type = Enum.TryParse<ActionItemType>(
                        formattedData.TryGetProperty("type", out var typeElement)
                            ? typeElement.GetString() ?? "Task"
                            : "Task", out var type) ? type : ActionItemType.Task,
                    Labels = formattedData.TryGetProperty("labels", out var labelsElement) && labelsElement.ValueKind == JsonValueKind.Array
                        ? labelsElement.EnumerateArray().Select(l => l.GetString()).Where(l => l != null).Cast<string>().ToArray()
                        : null
                };
            }
            catch (Exception ex)
