    ...
    public class TranscriptProcessorService
    {
        ...
        public async Task<MeetingTranscript> ProcessTranscriptAsync(string filePath)
        {
            try
            {
                ...
                Console.WriteLine($"✅ Processed transcript: {transcript.ActionItems.Count} action items found");
        ...
        private static async Task<string> ReadXmlContentAsync(string filePath)
        {
            try
            {
                ...
                var textPattern = @"<(?:text|content|transcript)[^>]*>(.*?)</(?:text|content|transcript)>";
                var matches = Regex.Matches(xmlContent, textPattern, RegexOptions.IgnoreCase | RegexOptions.Singleline);
        ...
        private void ExtractMetadata(MeetingTranscript transcript)
        {
            try
            {
                ...
                if (lines.Length > 0)
                {
                    transcript.Title = ExtractTitle(lines[0]) ?? Path.GetFileNameWithoutExtension(transcript.FileName);
                }
                ...
                transcript.ProjectKey = ExtractProjectKey(transcript.FileName + " " + transcript.Content);

                _logger?.LogInformation($"Extracted metadata - Title: {transcript.Title}, Participants: {transcript.Participants.Count}");
            }
            catch (Exception ex)
            {
...
";
        ...
        /// </summary>
        private static List<ActionItem> ParseActionItemsFromAIResponse(string aiResponse, MeetingTranscript transcript)
        {
            try
            {
                ...
                if (response?.actionItems != null)
                {
                    foreach (var item in response.actionItems)
                    {
                        ...
                        // Parse priority
                        if (Enum.TryParse<ActionItemPriority>(item.priority?.ToString() ?? "Medium", out ActionItemPriority priority))
                        {
                            actionItem.Priority = priority;
                        }

                        // Parse type
                        if (Enum.TryParse<ActionItemType>(item.type?.ToString() ?? "Task", out ActionItemType type))
                        {
        ...
        private static bool IsActionItemLine(string line)
        {
            ...
            return actionPatterns.Any(pattern =>
        ...
        private static ActionItemType DetermineActionItemType(string content)
        {
            ...
            if (lowerContent.Contains("bug") || lowerContent.Contains("fix") || lowerContent.Contains("error"))
            ...
            if (lowerContent.Contains("document") || lowerContent.Contains("write") || lowerContent.Contains("spec"))
        ...
        private static string? ExtractTitle(string firstLine)
        {
            ...
            var title = Regex.Replace(firstLine, @"^(meeting|transcript|notes?):\s*", "", RegexOptions.IgnoreCase).Trim();
