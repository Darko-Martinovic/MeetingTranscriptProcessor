...
namespace MeetingTranscriptProcessor.Services
{
    ...
    public class AzureOpenAIService : IDisposable
    {
        ...
        public AzureOpenAIService(ILogger? logger = null)
        {
            ...
            _apiKey = Environment.GetEnvironmentVariable("AOAI_APIKEY") ??
                     Environment.GetEnvironmentVariable("AZURE_OPENAI_API_KEY");
            _deploymentName = Environment.GetEnvironmentVariable("CHATCOMPLETION_DEPLOYMENTNAME") ??
                             Environment.GetEnvironmentVariable("AZURE_OPENAI_DEPLOYMENT_NAME") ?? "gpt-4";
            _logger = logger;
        ...
        public async Task<string> ProcessTranscriptAsync(string prompt)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    _logger?.LogError(new Exception($"Azure OpenAI API error: {response.StatusCode}"), errorContent);
        ...
        /// </summary>
        public async Task<string> FormatJiraTicketAsync(string actionItemTitle, string actionItemDescription, string meetingContext, string participants)
        {
            if (!IsConfigured())
            {
                _logger?.LogWarning("Azure OpenAI not configured, using fallback formatting");
                return await FormatJiraTicketFallbackAsync(actionItemTitle, actionItemDescription, meetingContext);
            }
...
";
        ...
        /// </summary>
        private async Task<string> FormatJiraTicketFallbackAsync(string title, string description, string context)
        {
            ...
            return JsonSerializer.Serialize(formattedTicket, new JsonSerializerOptions { WriteIndented = true });
        }
        ...
        {
            var keywords = new[] { "action", "todo", "task", "implement", "create", "fix", "review", "update", "investigate" };
        ...
        private static string DeterminePriority(string line)
        {
            ...
            if (lowerLine.Contains("urgent") || lowerLine.Contains("critical") || lowerLine.Contains("high"))
                return "High";
